...
Toy::Toy(const std::string& firm, const int cost, AgeLimits ageLimitsKids) :
	m_firm(firm), m_cost(cost), m_ageLimitsKids(ageLimitsKids) {}
...
//Methods
void Toy::findFirmCostByAgeLimits(AgeLimits ageLimits) {
	AgeLimits mainAgeLimits = m_ageLimitsKids;
	if ((mainAgeLimits.m_startLimitAge == ageLimits.m_startLimitAge) && (mainAgeLimits.m_endLimitAge == ageLimits.m_endLimitAge)) {
		std::cout << "Firm name: " << m_firm << std::endl;
